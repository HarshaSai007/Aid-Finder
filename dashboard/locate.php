...
while ($row = $result1->fetch_assoc()) {
	$a_name = $row['a_name'];
	$a_type = $row['a_type'];
	$a_mobile = $row['a_moblie'];
	$a_lat = $row['a_lat'];
	$a_lon = $row['a_lon'];
}

$sql2 = "SELECT * FROM `gps_track` WHERE `rider_id` = '$rider_id'";
...
$sql3 = "SELECT * FROM `agencies` WHERE 6371 * ACOS( SIN(RADIANS(a_lat)) * SIN(RADIANS($lat)) + COS(RADIANS(a_lat)) * COS(RADIANS($lat)) * COS(RADIANS(a_lon - ($lon)))) <= 20";
$result3 = mysqli_query($con, $sql3);

$agencies = [];

function filterAgencies($category, $agencies)
{
	$filteredAgencies = [];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	$categoryToFilter = $_POST['category'];
	$filteredAgencies = filterAgencies($categoryToFilter, $agencies);
}
...
<body>
					...
					<ul>
						...
						<li class="submenu active">
							...
							<ul>
								...
								<li><a href="locate.php" class="active">Locate</a></li>
				...
				<div class="row">
					...
					<div class="card flex-fill student-space comman-shadow">
						<div class="card-header d-flex align-items-center">
							<h5 class="card-title">Agencies Near Me</h5>
							...
							<div class="table-responsive">
								<table class="table star-student table-hover table-center table-borderless table-striped">
									...
									<tbody>
										...
										if ($filteredAgencies) {
											echo '<p> hello</p>';
											foreach ($filteredAgencies as $row) {
												...
												echo ' </td>';
												echo '<td class="text-center"><button class="btn btn-sm btn-danger" onclick="call(\'' . $row['a_moblie'] . ', ' . $row['a_name'] . '\')">Call</button></td>';
												echo '</tr>';
											}
										} else {
  ...
  <button 
    ...
    data-mobile="' . $row['a_moblie'] . '"
    data-name="' . $row['a_name'] . '"
    onclick="call(this)">
    Call
  </button>
</td>';
